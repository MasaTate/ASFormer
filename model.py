...
class AttLayer(nn.Module):
    def __init__(
        self, q_dim, k_dim, v_dim, r1, r2, r3, bl, stage, att_type, device
    ):  # r1 = r2
    ...
    def _block_wise_self_att(self, q, k, v, mask):
        ...
        if L % self.bl != 0:
            q = torch.cat(
                [
                    q,
                    torch.zeros((m_batchsize, c1, self.bl - L % self.bl)).to(
                        self.device
                    ),
                ],
                dim=-1,
            ...
            k = torch.cat(
                [
                    k,
                    torch.zeros((m_batchsize, c2, self.bl - L % self.bl)).to(
                        self.device
                    ),
                ],
                dim=-1,
            ...
            v = torch.cat(
                [
                    v,
                    torch.zeros((m_batchsize, c3, self.bl - L % self.bl)).to(
                        self.device
                    ),
                ],
                dim=-1,
